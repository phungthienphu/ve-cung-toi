...
export function RoleQuickView({ role, onClose }: { role: WerewolfRole; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Vai của tôi">
      ...
      <section className="relative z-10 w-full max-w-sm rounded-xl border border-[var(--ww-border)] bg-[var(--ww-surface-strong)] p-4 text-center shadow-2xl">
        <RoleArtwork role={role} className="mx-auto max-h-[68vh] w-auto rounded-md" />
        ...
        <button onClick={onClose} className="mt-4 w-full rounded-md bg-[var(--ww-accent-strong)] py-3 font-semibold text-[var(--ww-accent-ink)] transition hover:opacity-90">Đóng lại</button>
      </section>
    </div>
  );
}
